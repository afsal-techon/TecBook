    ...
    constructor() {
        ...
        this.getAllExpenses = async (req, res, next) => {
            try {
                ...
                const countResult = await ExpenseModel_1.ExpenseModel.aggregate(countPipeline);
                const totalCount = countResult[0]?.total || 0;
                pipeline.push({ $sort: { createdAt: -1 } }, { $skip: skip }, { $limit: limit });
                const expenses = await ExpenseModel_1.ExpenseModel.aggregate(pipeline);
                res.status(http_status_1.HTTP_STATUS.OK).json({
                    success: true,
                    data: expenses,
                    pagination: {
        ...
        this.projectModel = project_1.default;
    }
